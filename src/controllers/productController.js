...
const getProduct = async function (req, res) {
  try {
    ...
    let data = req.query;
    let limitValue = req.query.limit || 2
    ...
    const getData = await productModel.find(filter).skip(limitValue*(page-1)).limit(limitValue).sort({ price: priceSort }).select({ __v: 0 });
